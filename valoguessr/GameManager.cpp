...
void GM::filterMaps()
{
	GM::getInstance().filteredMaps.clear();

	for (const Map& map : GM::getInstance().allMaps[GM::getInstance().config.difficulty - 1])
	{
		switch (map.mapType)
		{
		case MapEnum::ASCENT: // ascent
			if (GM::getInstance().config.mapChoice[0])
				GM::getInstance().filteredMaps.push_back(map);
			break;
	...
	for (Map& map : GM::getInstance().filteredMaps)
...
{
	for (size_t i = 0; i < sizeof(GM::getInstance().allMaps[0]) / sizeof(GM::getInstance().allMaps[0][0]); i++)
	{
		if (strlen(mapPath) != strlen(GM::getInstance().allMaps[difficulty - 1][i].mapPath))
			continue; 
		...
		{
			if ((char)mapPath[strInd] != (char)GM::getInstance().allMaps[difficulty - 1][i].mapPath[strInd])
			{
				match = false;
...
void GM::newGame()
{
	GM::getInstance().usedMaps.clear();
	GM::getInstance().filterMaps();
	...
	for (uint8_t i = 0; i < 5; i++)
	{
		...
		do
		{
			mapIndex = rand() % GM::getInstance().filteredMaps.size();
		} while (std::count(GM::getInstance().usedMaps.begin(), GM::getInstance().usedMaps.end(), mapIndex));

		GM::getInstance().usedMaps.push_back(mapIndex);
		reservedMaps.push_back(GM::getInstance().filteredMaps[mapIndex]);
	}
	...
	GM::getInstance().gameSlot = new Game(reservedMaps, GM::getInstance().config.difficulty, GameType::STANDARD);
}
...
void GM::newFreeplay()
{
	...
	if (GM::getInstance().filteredMaps.size() < 1)
	{
#ifdef _DEBUG
	...
	randomizedMaps.reserve(GM::getInstance().usedMaps.size());

	for (uint8_t i = 0; i < GM::getInstance().filteredMaps.size(); i++)
	{
		unsigned mapIndex;
		do
		{
		...
		GM::getInstance().usedMaps.push_back(mapIndex);
		randomizedMaps.push_back(GM::getInstance().filteredMaps[mapIndex]);
	}
	...
	GM::getInstance().gameSlot = new Game(randomizedMaps, GM::getInstance().config.difficulty, GameType::FREEPLAY);
}
...
void GM::newImport(std::string importString)
{
	...
	for (size_t curr = 0; curr < importString.size(); curr++)
	{
		if (importString[curr] == ';')
		{
			if (!prev) // first slice: difficulty
				...
				maps.push_back({ GM::getInstance().allMaps[difficulty - 1][std::stoul(importString.substr(prev, curr - prev), nullptr, 16)] });
	...
	GM::getInstance().importedPoints = points;
	GM::getInstance().config.difficulty = difficulty;
	GM::getInstance().gameSlot = new Game(maps, difficulty, GameType::IMPORT);
}
...
{
	if (save)
		GM::getInstance().config.games.push_back(*GM::getInstance().gameSlot);

	delete GM::getInstance().gameSlot;
	GM::getInstance().gameSlot = nullptr;
}
...
void GM::calcStats()
{
	size_t numOfStandards = GM::getInstance().config.games.size();
	for (size_t i = 0; i < GM::getInstance().config.games.size(); i++)
		if (GM::getInstance().config.games[i].gameType == GameType::IMPORT)
			numOfStandards--;

	if (!numOfStandards)
		return;

	uint32_t low = GM::getInstance().config.games[0].sumOfRounds();
	...
	if (GM::getInstance().config.games.size() > 1)
	{
		for (size_t i = 1; i < GM::getInstance().config.games.size(); i++)
		{
			if (GM::getInstance().config.games[i].gameType == GameType::IMPORT)
				continue;

			total += GM::getInstance().config.games[i].sumOfRounds();

			if (GM::getInstance().config.games[i].sumOfRounds() > max)
				max = GM::getInstance().config.games[i].sumOfRounds();
			else if (GM::getInstance().config.games[i].sumOfRounds() < low)
				low = GM::getInstance().config.games[i].sumOfRounds();

			if (GM::getInstance().config.games[i].sumOfRounds() >= 20000)
				Ws++;
			else
				Ls++;
		}
	...
	GM::getInstance().config.playCount = (uint16_t)numOfStandards;

	GM::getInstance().config.pointRecord = max;
	GM::getInstance().config.lowestPoints = low;
	GM::getInstance().config.totalPoints = total;

	GM::getInstance().config.Ws = Ws;
	GM::getInstance().config.Ls = Ls;
}

void GM::calcAchievements()
{
	if (GM::getInstance().config.playCount >= 5)
		GM::getInstance().config.achievements[1] = 0b1;
	else
		GM::getInstance().config.achievements[1] = 0b0;

	if (GM::getInstance().config.playCount >= 10)
		GM::getInstance().config.achievements[2] = 0b1;
	else
		GM::getInstance().config.achievements[2] = 0b0;

	if (GM::getInstance().config.playCount >= 15)
		GM::getInstance().config.achievements[3] = 0b1;
	else
		GM::getInstance().config.achievements[3] = 0b0;

	if (GM::getInstance().config.pointRecord > 10000)
		GM::getInstance().config.achievements[4] = 0b1;
	else
		GM::getInstance().config.achievements[4] = 0b0;

	if (GM::getInstance().config.pointRecord > 15000)
		GM::getInstance().config.achievements[5] = 0b1;
	else
		GM::getInstance().config.achievements[5] = 0b0;

	if (GM::getInstance().config.pointRecord > 20000)
		GM::getInstance().config.achievements[6] = 0b1;
	else
		GM::getInstance().config.achievements[6] = 0b0;

	if (GM::getInstance().config.pointRecord == 25000)
		GM::getInstance().config.achievements[7] = 0b1;
	else
		GM::getInstance().config.achievements[7] = 0b0;
}

void GM::init()
{
	GM::getInstance().filteredMaps.reserve(48);
	GM::getInstance().usedMaps.reserve(15);
}

void GM::cleanup()
{
	if (GM::getInstance().gameSlot)
		delete GM::getInstance().gameSlot;
}
...
{
	this->filteredMaps.reserve(48);
	this->usedMaps.reserve(15);
}
